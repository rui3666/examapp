				...
				<scroll-view scroll-y :style="'height:'+sHeight+'px'" style="width: 100%;">
					<view class="c_fir">
						<view class="cf_left">
							...
							<text class="cl_text">--------</text>
							<text class="cl_text" @click="shoucangtimu(item.timuId)">收藏题目</text>
						</view>
						<view class="cf_right">
							<text class="cr_text1" v-text="index+1"></text>
							<!-- <text class="cr_text2" v-text="'/'+examTotal"></text> -->
							<text class="cr_text2">/{{list.length}}</text>
						</view>
					</view>
					<view class="c_sec">
						<view class="q_title">
						...
						<view class="q_choose">
							...
							<view class='qc_dx' v-if="item.type==0">
								<radio-group @change='answerChange'>
									<label class="pd_item" v-for="(it,index2) in item.answerList" :key='index2'>
										<view class="pdi_left">
											<radio :value="it.value" :checked="item.userAnswer==it.value?true:false"
												style="transform:scale(0.8)" color="#62B6FF"></radio>
										</view>
										<view class="pdi_right" v-html="it.label"></view>
									</label>
								</radio-group>
							</view>
							...
							<view class="qc_dx" v-if="item.type==1">
								<checkbox-group @change='answerChange'>
									<label class="pd_item" v-for="(it,index2) in item.answerList" :key='index2'>
										<view class="pdi_left">
											<checkbox :value="it.value" style="transform:scale(0.8)"
												:checked="item.userAnswer.indexOf(it.value)==-1?false:true"
												color="#62B6FF"></checkbox>
										</view>
					...
					<view class="c_thi" v-if="issubmit">
						<view class="ct_fir">
							...
							<view class="ctf_item">你的答案： <text :class="item.answer==item.userAnswer?'ci_c':'ci_w'"
									v-text="item.userAnswer"></text></view>
						</view>
						<view class="ct_sec">
							<view class="cts_left">
						...
						<u-parse :content="item.example" :imageProp="imgprop" :noData="noData" />
					</view>
				</scroll-view>
				...
				<scroll-view style="width: 100%;height: 71vh;" scroll-y>
					<view class="d_con">
						<view class="dc_item" v-for="(item,index) in examList" :key='index'>
							<view @click="nowexamchangeByDTK(index)"
								:class="item.userAnswer==''?(tempUserAnswer==''?'dci_num2':(nowexam==index?'dci_num':'dci_num2')):'dci_num'"
								v-text="index+1"></view>
						</view>
					</view>
				</scroll-view>
...
<script>
	...
	export default {
		...
		mounted() {
			var isFlag = uni.getStorageSync('isFlag');
			uni.removeStorageSync("isFlag");
			if (isFlag) {
				this.issubmit = true;
			}
			this.getData();
		...
		methods: {
			shoucangtimu(TKiD) {
				...
				var x = this;
				uni.request({ 
					url: 'http://127.0.0.1:9001/birthdaydemo/YongHu/updateTk',
					method: 'PUT',
					header: {
			...
			nowchangeBybtn(num) {
				...
				this.examTotal = this.list.length;
			},
				...
				if (this.issubmit == false) {
					...
					if (this.sutype == 0) {
						...
						for (var i = 0; i < this.examList.length; i++) {
							...
							this.chooseUserAnswer[i] = this.examList[i].answer;
						}
						this.$emit('getLastAnswer', this.lastUserAnswer);
						this.$emit('getChooseUserAnswer', this.chooseUserAnswer);
					}
					this.issubmit = true;
					this.$refs.popup.close();
					...
					uni.request({ //发送请求
						...
						success: (res1) => {
							uni.request({ //发送请求
								...
								data: {
									yhOpenId: openId,
									yhIntegrate: parseInt(res1.data.data.yhIntegrate) + 50,
								},
								success: (res) => {
									console.log(res);
								},
